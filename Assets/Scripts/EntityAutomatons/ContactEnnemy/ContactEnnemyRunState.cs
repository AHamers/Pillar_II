...
public class ContactEnnemyRunState : EntityState
{
    private GameObject target;

    public ContactEnnemyRunState(GameObject target)
    {
        this.target = target;
    }
    public override EntityState checkTransition()
    {
        if((target.transform.position - automaton.transform.position).magnitude > automaton.gameObject.GetComponent<ContactEnnemySettings>().targetDetectionDistance)
        {
            return new ContactEnnemyIdleState();
        }
    ...
    public override void fixedUpdateState()
    {
        ...
        if (hasFoundPath)
        {
            ...
            automaton.GetComponent<NavMeshAgent>().Move((path.corners[1] - automaton.transform.position).normalized * automaton.GetComponent<ContactEnnemySettings>().runningSpeed * Time.deltaTime);
        }
    ...
    {
        this.automaton.GetComponent<Animator>().SetInteger("AnimationState", (int)EntityAnimationState.RUNNING);
